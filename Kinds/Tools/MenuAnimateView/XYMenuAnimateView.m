...
- (void)show {
    ...
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.05 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        for (int i = 0; i<self.imageNameArray.count; i++) {
            ...
            button.frame = CGRectMake(edge+(edge+width)*loc, SCREEN_HEIGHT+edge+(edge+width-10)*row, width, width);
            ...
            [UIView animateWithDuration:2 delay:i*3 usingSpringWithDamping:0.7 initialSpringVelocity:0.05 options:UIViewAnimationOptionAllowUserInteraction animations:^{
