...
- (instancetype)initWithFrame:(CGRect)frame {
    ...
    self.backgroundColor = RGB_alpha(243, 243, 243, 1.0);
...
- (void)setIsEditing:(BOOL)isEditing {
    ...
    if (_isEditing) {
        [UIView animateWithDuration:0.2 animations:^{
            ...
            self.rightButton.x = SCREEN_WIDTH - 38 - 40;
            self.backgroundImageView.width = SCREEN_WIDTH - 20 - 40;
            self.cancelButton.x = SCREEN_WIDTH - 40;
        } completion:^(BOOL finished) {
            self.searchTextField.width = SCREEN_WIDTH - 20 - 38 - 40;
        }];
        ...
        [UIView animateWithDuration:0.2 animations:^{
            self.searchTextField.x = SCREEN_WIDTH * 0.5 - 40;
            self.rightButton.x = SCREEN_WIDTH - 38;
            self.backgroundImageView.width = SCREEN_WIDTH - 20;
            self.cancelButton.x = SCREEN_WIDTH;
        } completion:^(BOOL finished) {
            self.searchTextField.width = SCREEN_WIDTH * 0.5 + 20 - 38;
        }];
...
- (BOOL)textFieldShouldEndEditing:(UITextField *)textField {
    [UIView animateWithDuration:0.3 animations:^{
        ...
        self.searchTextField.width = SCREEN_WIDTH * 0.5 + 20 - 38 - 40;
    }];
    return YES;
}
...
- (UIImageView *)backgroundImageView {
    if (!_backgroundImageView) {
        ...
        _backgroundImageView.frame = CGRectMake(10, 8, SCREEN_WIDTH - 20, 44 - 16);
    }
...
- (UITextField *)searchTextField {
    if (!_searchTextField) {
        _searchTextField = [[JKRSearchTextField alloc] initWithFrame:CGRectMake(SCREEN_WIDTH * 0.5 - 40, 0, SCREEN_WIDTH * 0.5 + 20 - 38, 44)];
...
- (UIButton *)rightButton {
    if (!_rightButton) {
        ...
        _rightButton.frame = CGRectMake(SCREEN_WIDTH - 38, 8, 28, 28);
    }
    return _rightButton;
}
...
- (UIButton *)cancelButton {
    if (!_cancelButton) {
        ...
        _cancelButton.frame = CGRectMake(SCREEN_WIDTH, 0, 40, 44);
        _cancelButton.titleLabel.font = [UIFont systemFontOfSize:16];
        [_cancelButton setTitle:@"取消  " forState:UIControlStateNormal];
        [_cancelButton setTitleColor:RGB_alpha(85, 183, 55, 1.0) forState:UIControlStateNormal];
